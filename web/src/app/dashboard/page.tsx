...
const Slider = () => {
  const slides = [
    { image: "/images/BANNERDENIM.jpeg", title: "", subtitle: "" },
    { image: "/images/BANNERDENIM.jpeg", title: "", subtitle: "" },
  ];
...
export default function DashboardPage() {
  const [isSliderVisible] = useState(true);
  const [points] = useState(400);
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <Link href="/orders" className="w-full text-center border-2 border-red-600 font-bold py-3 px-4 rounded-lg hover:bg-gray-100 transition text-gray-800">
          Seguimiento de pedido
        </Link>
        <Link href="/prizes" className="w-full text-center border-2 border-red-600 font-bold py-3 px-4 rounded-lg hover:bg-gray-100 transition flex justify-center items-center text-gray-800">
          <span>Puntos</span>
          <span className="ml-4 bg-red-600 text-white text-sm font-bold px-3 py-1 rounded-full">{points}</span>
        </Link>
      </div>
